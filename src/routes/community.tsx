      ...
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
          ...
          <div className="grid md:grid-cols-3 gap-3">
            {(myDistrict.length ? myDistrict : projects.slice(0, 6)).map((p) => (
              <div key={p.id} className="bg-card border rounded-xl p-4 shadow-card">
                ...
                <button className="w-full bg-saffron text-white text-xs py-2 rounded-lg font-semibold">
                  Claim This Project
                </button>
              </div>
            ))}
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="bg-success text-white px-4 py-2 rounded-lg text-sm font-semibold inline-flex items-center gap-1">
                ...
                <div className="space-y-2">
                  {(p.proposed_use as any[]).map((u, i) => (
                    <div key={i} className="text-xs bg-muted/50 p-2 rounded">
                      <span className="font-semibold">{u.title}</span> — ₹{u.estimated_cost}L
                    </div>
                  ))}
                </div>
              </div>
