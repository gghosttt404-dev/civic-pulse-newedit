...
function Reports() {
  ...
  useEffect(() => {
    ...
    supabase.from("schemes").select("id", { count: "exact", head: true }).then(({ count }) => setSchemeCount(count || 0));
  }, []);

  const filtered = projects.filter(p => p.state === state && (!district || p.district.toLowerCase().includes(district.toLowerCase())));
  const ghosts = filtered.filter(p => p.ghost_score > 55);
  const totalWaste = ghosts.reduce((s, p) => s + Number(p.sanctioned_amount), 0);

  const districtCards = Array.from(new Set(projects.map(p => `${p.district}|${p.state}`))).map(s => {
    const [d, st] = s.split("|");
    const ps = projects.filter(p => p.district === d);
    return { d, st, count: ps.length, ghosts: ps.filter(p => p.ghost_score > 55).length };
  }).slice(0, 8);
    ...
    <AppShell>
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Generate evidence-backed civic intelligence reports for any district.</p>
        </div>
        ...
        <div className="bg-card border rounded-xl p-5 shadow-card flex flex-wrap gap-3">
          <select value={state} onChange={e => setState(e.target.value)} className="px-3 py-2 border rounded-lg text-sm">
            {["Bihar","Maharashtra","Uttar Pradesh","Jharkhand","Madhya Pradesh","Rajasthan","Karnataka"].map(s => <option key={s}>{s}</option>)}
          </select>
        ...
        {filtered.length > 0 && (
          <div className="bg-card border rounded-2xl p-6 shadow-card">
            <h2 className="font-bold text-xl mb-1">Report: {district || "All districts"} • {state}</h2>
            <p className="text-xs text-muted-foreground mb-5">Compiled from satellite AI + government records</p>
            ...
            <div className="space-y-2 mb-5">
              {ghosts.slice(0, 5).map(p => (
                <div key={p.id} className="flex items-center justify-between bg-muted/40 p-3 rounded-lg text-sm">
                  <span className="font-medium">{p.name}</span>
                  <div className="flex items-center gap-3"><span>{formatINR(p.sanctioned_amount)}</span><SeverityBadge severity={p.severity} /></div>
                </div>
              ))}
            </div>
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-3">
            {districtCards.map(c => (
              <button key={c.d + c.st} onClick={() => { setState(c.st); setDistrict(c.d); }}
...
function Stat({ label, value, tone }: any) {
  ...
  return <div className="bg-muted/40 rounded-xl p-4"><div className={`text-3xl font-bold ${m[tone]}`}>{value}</div><div className="text-xs text-muted-foreground mt-1">{label}</div></div>;
}
