...
function Onboarding() {
  ...
  const submit = async () => {
    ...
    const { data: row, error } = await supabase
      .from("users")
      .insert({ ...data, profile_complete: true })
      .select()
      .single();
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-card rounded-2xl shadow-card p-6 lg:p-8">
          <div className="flex items-start gap-3 mb-6 p-3 bg-saffron/5 rounded-lg border border-saffron/20">
            <div className="w-8 h-8 rounded-full bg-saffron flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
              NB
            </div>
            <div className="text-sm text-foreground/80">{TIPS[step]}</div>
          </div>

          <div className="text-xs text-saffron font-semibold tracking-wider mb-2">
            STEP {step} OF 8
          </div>
          ...
          {step === 2 && (
            <Section title="Caste & Community">
              <Radio
                label="Caste Category"
                value={data.caste_category}
                onChange={(v) => update("caste_category", v)}
                options={["GEN", "OBC", "SC", "ST"]}
              />
            </Section>
          )}
          ...
          {step === 3 && (
            <Section title="Education">
              <Select
                ...
                options={["BELOW_10", "CLASS_10", "CLASS_12", "GRADUATE", "POSTGRAD", "PHD"]}
              />
              <Input
                label="Stream / Subject"
                value={data.education_stream}
                onChange={(v) => update("education_stream", v)}
              />
            </Section>
          )}
          ...
          {step === 4 && (
            <Section title="Occupation & Income">
              ...
              <Radio
                ...
                onChange={(v) => update("employer_type", v)}
                options={["SELF", "GOVT", "PRIVATE", "NONE"]}
              />
              <div>
                ...
                <input
                  ...
                  className="w-full accent-saffron mt-2"
                />
              </div>
            </Section>
          )}
          ...
          {step === 5 && (
            <Section title="Family">
              ...
              <Toggle
                label="BPL Status"
                value={data.bpl_status}
                onChange={(v) => update("bpl_status", v)}
              />
              <Select
                label="Ration Card Type"
                value={data.ration_card_type}
                onChange={(v) => update("ration_card_type", v)}
                options={["APL", "BPL", "AAY", "NONE"]}
              />
            </Section>
          )}
          ...
          {step === 6 && (
            <Section title="Assets">
              <Input
                ...
                type="number"
                value={data.land_holding_acres}
                onChange={(v) => update("land_holding_acres", +v)}
              />
              <Select
                label="Bank Account Type"
                value={data.bank_account_type}
                onChange={(v) => update("bank_account_type", v)}
                options={["SAVINGS", "JAN_DHAN", "CURRENT", "NONE"]}
              />
            </Section>
          )}

          {step === 7 && (
            <Section title="Health & Special">
              <Toggle
          ...
          <div className="flex justify-between mt-8">
            ...
            {step < 8 ? (
              <button
                onClick={() => setStep(step + 1)}
                className="bg-navy-deep text-white px-5 py-2.5 rounded-lg text-sm font-semibold inline-flex items-center gap-2"
              >
              ...
              <button
                onClick={submit}
                className="bg-saffron text-white px-6 py-3 rounded-lg text-sm font-semibold inline-flex items-center gap-2"
              >
                <Check className="w-4 h-4" /> Find My Schemes
              </button>
            )}
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-saffron outline-none"
      />
    </div>
  );
}
      ...
      <div className="flex flex-wrap gap-2">
        {options.map((o) => (
          <button
            ...
            type="button"
            onClick={() => onChange(o)}
            className={`px-4 py-2 rounded-full text-sm border ${value === o ? "bg-navy-deep text-white border-navy-deep" : "bg-white hover:border-saffron"}`}
          >
            {o}
          </button>
        ))}
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <button
        ...
        className={`w-11 h-6 rounded-full relative transition ${value ? "bg-saffron" : "bg-muted"}`}
      >
        <span
          className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition ${value ? "left-5" : "left-0.5"}`}
        />
      </button>
    </div>
  );
