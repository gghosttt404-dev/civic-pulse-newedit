...
import { SAMPLE_PROJECTS, type Project } from "@/lib/sample-projects";
...
function ProjectDetail() {
  ...
  useEffect(() => {
    supabase
      ...
      .maybeSingle()
      .then(({ data }) => {
        setP(data ?? SAMPLE_PROJECTS.find((project) => project.id === id) ?? null);
      });
    supabase
          ...
          <div className="bg-[oklch(0.15_0.02_250)] text-white p-7">
            ...
            <div className="flex items-center gap-2">
              ...
              {p.construction_detected ? (
                <span className="bg-success text-white text-xs font-bold px-3 py-1 rounded-full inline-flex items-center gap-1">
                  <Check className="w-3 h-3" /> YES
                </span>
              ) : (
                <span className="bg-danger text-white text-xs font-bold px-3 py-1 rounded-full inline-flex items-center gap-1">
